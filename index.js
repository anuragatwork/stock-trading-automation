...
console.log("main server started running");
...
//run this function for cron job ( during market hours )
async function performTask(){
    
    //sell all positions that were bought prior 
    const liquidate=await alpaca.closeAllPositions();
    console.log("positions liquidated ",liquidate);
...
// Schedule task to start server at 9:30 AM ET (UTC-5/UTC-4)
cron.schedule('30 9 * * 1-5', () => {
  console.log('Starting server at 9:30 AM ET');
  startServer();
}, {
  timezone: 'America/New_York'
});

// Schedule task to stop server at 4:00 PM ET (UTC-5/UTC-4)
cron.schedule('00 16 * * 1-5', () => {
  console.log('Stopping server at 4:00 PM ET');
  stopServer();
}, {
  timezone: 'America/New_York'
});
